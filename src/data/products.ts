import type { Product } from "@/types";

export const products: Product[] = [
  {
    name: "Foundation خانم خانوما",
    oldPrice: "249.95",
    price: "200.95",
    id: "12132klj2322",
    isSale: true,
    isNew: false,
    image: "/assets/img/product-img2.jpg",
    category: "makeup",
    filterItems: ["nail", "skin", "makeup", "spa", "perfume", "hair"],
    isStocked: true,
    productNumber: "IN1203",
    imageGallery: [
      "/assets/img/product-img2.jpg",
      "/assets/img/product-img6.jpg",
      "/assets/img/product-img7.jpg",
      "/assets/img/product-img8.jpg",
      "/assets/img/product-img9.jpg",
    ],
    colors: ["#FCEDEA", "#FEE1DB", "#FFD9D1", "#FDC5B9", "#FDB7A8", "#FFA08A"],
    content:
      "Universal foundation masks skin imperfections and gives it a matte finish, remaining completely invisible on the face. The foundation is suitable for any skin type. Thanks to a unique combination of pigments, the foundation adapts perfectly to even the smallest features of skin tone, creating an even and natural tone",
    description:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Modi beatae provident ipsum omnis dolor sapiente maiores reiciendis exercitationem earum deleniti, reprehenderit iste ipsa saepe. Consectetur non et excepturi molestias esse?",
    reviews: [
      {
        author: {
          image: "/assets/img/comment-author1.jpg",
          name: "Melissa Jones",
        },
        reviewDate: "July 23, 2020",
        rating: 4,
        content:
          "I am grateful to the employees of خانم خانوما for the quality products that I have been using for more than a year, try to work at this level in the future. Thank you",
      },
      {
        author: {
          image: "/assets/img/comment-author3.jpg",
          name: "Amanda Clement",
        },
        reviewDate: "June 15, 2020",
        rating: 5,
        content:
          "I am grateful to the employees of خانم خانوما for the quality products that I have been using for more than a year, try to work at this level in the future. Thank you). I am grateful to the employees of خانم خانوما for the quality products.",
      },
      {
        author: {
          image: "/assets/img/comment-author2.jpg",
          name: "Steve Gentley",
        },
        reviewDate: "July 05, 2020",
        rating: 3,
        content:
          "I am grateful to the employees of خانم خانوما for the quality products that I have been using for more than a year, try to work at this level in the future. Thank you",
      },
    ],
  },
  {
    name: "Lotion For Cleansing",
    oldPrice: "149.50",
    price: "100.90",
    id: "12132klj1213",
    isSale: true,
    isNew: false,
    image: "/assets/img/product-img1.jpg",
    category: "spa",
    filterItems: ["makeup", "spa", "perfume", "hair"],
    isStocked: true,
    productNumber: "IN1203",
    imageGallery: [
      "/assets/img/product-img1.jpg",
      "/assets/img/product-img6.jpg",
      "/assets/img/product-img7.jpg",
      "/assets/img/product-img8.jpg",
      "/assets/img/product-img9.jpg",
    ],
    colors: ["#FCEDEA", "#FEE1DB", "#FFD9D1", "#FDC5B9", "#FDB7A8", "#FFA08A"],
    content:
      "Universal foundation masks skin imperfections and gives it a matte finish, remaining completely invisible on the face. The foundation is suitable for any skin type.",
    description:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Modi beatae provident ipsum omnis dolor sapiente maiores reiciendis exercitationem earum deleniti.",
    reviews: [],
  },
  {
    name: "Tony Mask",
    oldPrice: "549.95",
    price: "200.95",
    id: "12132klj232453",
    isSale: false,
    isNew: true,
    image: "/assets/img/product-img3.jpg",
    category: "nail",
    filterItems: ["nail", "skin", "makeup", "spa"],
    isStocked: true,
    productNumber: "IN1203",
    imageGallery: [
      "/assets/img/product-img3.jpg",
      "/assets/img/product-img6.jpg",
      "/assets/img/product-img7.jpg",
      "/assets/img/product-img8.jpg",
      "/assets/img/product-img9.jpg",
    ],
    colors: ["#FCEDEA", "#FEE1DB", "#FFD9D1", "#FDC5B9", "#FDB7A8", "#FFA08A"],
    content:
      "Universal foundation masks skin imperfections and gives it a matte finish, remaining completely invisible on the face.",
    description:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Modi beatae provident ipsum omnis dolor sapiente.",
    reviews: [],
  },
  {
    name: "Spray Balm With Oat Extract",
    oldPrice: "249.95",
    price: "200.95",
    id: "12132klj2121212",
    isSale: true,
    isNew: false,
    image: "/assets/img/product-img4.jpg",
    category: "hair",
    filterItems: ["nail", "makeup", "perfume", "hair"],
    isStocked: true,
    productNumber: "IN1203",
    imageGallery: [
      "/assets/img/product-img4.jpg",
      "/assets/img/product-img6.jpg",
      "/assets/img/product-img7.jpg",
      "/assets/img/product-img8.jpg",
      "/assets/img/product-img9.jpg",
    ],
    colors: ["#FCEDEA", "#FEE1DB", "#FFD9D1", "#FDC5B9", "#FDB7A8", "#FFA08A"],
    content:
      "Spray balm with oat extract nourishes and protects your hair, leaving it soft and manageable.",
    description:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Modi beatae provident ipsum omnis dolor sapiente maiores reiciendis.",
    reviews: [],
  },
  {
    name: "Hydrogel Patches",
    oldPrice: "249.95",
    price: "200.95",
    id: "12132klj24444",
    isSale: true,
    isNew: false,
    image: "/assets/img/product-img5.jpg",
    category: "perfume",
    filterItems: ["makeup", "spa", "perfume", "hair"],
    isStocked: true,
    productNumber: "IN1203",
    imageGallery: [
      "/assets/img/product-img5.jpg",
      "/assets/img/product-img6.jpg",
      "/assets/img/product-img7.jpg",
      "/assets/img/product-img8.jpg",
      "/assets/img/product-img9.jpg",
    ],
    colors: ["#FCEDEA", "#FEE1DB", "#FFD9D1", "#FDC5B9", "#FDB7A8", "#FFA08A"],
    content:
      "Hydrogel patches reduce puffiness and dark circles, leaving your under-eye area refreshed and youthful.",
    description:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Modi beatae provident ipsum omnis.",
    reviews: [],
  },
  {
    name: "Detox Body Cream",
    oldPrice: "249.95",
    price: "200.95",
    id: "12132klj2322321",
    isSale: true,
    isNew: true,
    image: "/assets/img/product-img6.jpg",
    category: "skin",
    filterItems: ["makeup", "skin", "perfume", "hair"],
    isStocked: true,
    productNumber: "IN1203",
    imageGallery: [
      "/assets/img/product-img6.jpg",
      "/assets/img/product-img5.jpg",
      "/assets/img/product-img7.jpg",
      "/assets/img/product-img8.jpg",
      "/assets/img/product-img9.jpg",
    ],
    colors: ["#FCEDEA", "#FEE1DB", "#FFD9D1", "#FDC5B9", "#FDB7A8", "#FFA08A"],
    content:
      "Detox body cream helps eliminate toxins and impurities, leaving your skin smooth and revitalized.",
    description:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Modi beatae provident ipsum omnis dolor sapiente maiores.",
    reviews: [],
  },
  {
    name: "Spray Balm With Oat Extract",
    oldPrice: "249.95",
    price: "200.95",
    id: "12132klj2121213",
    isSale: false,
    isNew: true,
    image: "/assets/img/product-img4.jpg",
    category: "hair",
    filterItems: ["nail", "makeup", "perfume", "hair"],
    isStocked: true,
    productNumber: "IN1204",
    imageGallery: [
      "/assets/img/product-img4.jpg",
      "/assets/img/product-img6.jpg",
      "/assets/img/product-img7.jpg",
      "/assets/img/product-img8.jpg",
      "/assets/img/product-img9.jpg",
    ],
    colors: ["#FCEDEA", "#FEE1DB", "#FFD9D1", "#FDC5B9", "#FDB7A8", "#FFA08A"],
    content:
      "Spray balm with oat extract nourishes and protects your hair, leaving it soft and manageable.",
    description:
      "Lorem ipsum dolor sit amet consectetur adipisicing elit. Modi beatae provident ipsum.",
    reviews: [],
  },
  {
    name: "Hydrogel Patches",
    oldPrice: "249.95",
    price: "200.95",
    id: "12132klj24445",
    isSale: false,
    isNew: true,
    image: "/assets/img/product-img5.jpg",
    category: "perfume",
    filterItems: ["makeup", "spa", "perfume", "hair"],
    isStocked: true,
    productNumber: "IN1205",
    imageGallery: [
      "/assets/img/product-img5.jpg",
      "/assets/img/product-img6.jpg",
      "/assets/img/product-img7.jpg",
      "/assets/img/product-img8.jpg",
      "/assets/img/product-img9.jpg",
    ],
    colors: ["#FCEDEA", "#FEE1DB", "#FFD9D1", "#FDC5B9", "#FDB7A8", "#FFA08A"],
    content:
      "Hydrogel patches reduce puffiness and dark circles, leaving your under-eye area refreshed.",
    description: "Lorem ipsum dolor sit amet consectetur adipisicing elit.",
    reviews: [],
  },
];

export const getProductById = (id: string): Product | undefined => {
  return products.find((product) => product.id === id);
};

export const getProductsByCategory = (category: string): Product[] => {
  return products.filter((product) => product.category === category);
};

export const getSaleProducts = (): Product[] => {
  return products.filter((product) => product.isSale);
};

export const getNewProducts = (): Product[] => {
  return products.filter((product) => product.isNew);
};

export const getTrendingProducts = (): Product[] => {
  return products.slice(0, 4);
};


